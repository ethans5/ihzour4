...
def load_corpus_data(
    ...
    fixed_emb_path = (
        base_path / "embeddings" / f"{corpus_name}_fixed_embeddings_mpnet.npy"
    )

    # Child chunks
    child_chunks_path = base_path / "chunks" / f"{corpus_name}_child_chunks.jsonl"
    child_emb_path = (
        base_path / "embeddings" / f"{corpus_name}_child_embeddings_mpnet.npy"
    )
...
def plot_corpus_histogram(chunks: List[Dict], corpus_name: str):
    ...
    plt.hist(
        timestamps,
        bins=range(min(timestamps), max(timestamps) + 2),
        edgecolor="black",
        alpha=0.7,
    )
    plt.title(
        f"Temporal Distribution of {corpus_name.upper()} Corpus ({valid_chunks} valid dates)"
    )
    plt.xlabel("Year")
...
def run_baseline_failures(
    retriever, queries: List[Dict], corpus_name: str, chunking: str, representation: str
):
        ...
        results.append(
            {
                ...
                "top_doc_preview": (
                    retrieved[0].get("text", "")[:150] if retrieved else "None"
                ),
            }
        )
...
def run_hard_filtering_tests(
    ...
    target_queries = [
        q for q in queries if q["type"] == "point_in_time" or "filter_date_range" in q
    ]
    ...
    for q_obj in target_queries:
        ...
        if "filter_year" in q_obj:
            retrieved = retriever.search(
                q_obj["query"], k=3, filter_year=q_obj["filter_year"]
            )
        elif "filter_date_range" in q_obj:
            retrieved = retriever.search(
                q_obj["query"], k=3, filter_date_range=q_obj["filter_date_range"]
            )
        else:
        ...
        if "filter_year" in q_obj:
            ...
            success = (
                all(str(filter_val) in d for d in dates if d != "N/A")
                or len(dates) == 0
            )
        else:
            success = len(dates) > 0
...
def run_recency_weighting_tests(
    retriever, queries: List[Dict], corpus_name: str, chunking: str, representation: str
):
        ...
        for alpha in alphas:
            for lambda_val in lambdas:
                ...
                config = {
                    ...
                    "retrieved_dates": dates,
                    "avg_year": (
                        np.mean([int(d[:4]) for d in dates if d]) if dates else 0
                    ),
                }
...
def main():
    ...
    print(
        f"  - Point-in-time: {len([q for q in ALL_QUERIES if q['type'] == 'point_in_time'])}"
    )
    print(
        f"  - Current status: {len([q for q in ALL_QUERIES if q['type'] == 'current'])}"
    )
    ...
    try:
        ...
        print(
            "Warning: evaluation.generate_comparison_plots not found or failed to import."
        )
