...
def generate_proof():
    ...
    baseline_entry = next((q for q in data["baseline"] if q["desc"] == QUERY_DESC), None)
    ...
    evolution_entry = next((q for q in data["evolution"] if q["desc"] == QUERY_DESC), None)
    ...
    all_evo_dates = sorted(evolution_entry["oldest_dates"] + evolution_entry["newest_dates"])
    ...
    rects1 = ax.bar(x - width/2, k3_data, width, label='K = 3 (Baseline)', color='#A9A9A9')
    rects2 = ax.bar(x + width/2, k_pool_data, width, label='Pool Evolution (K=50)', color='#44a1ad')
...
def generate_bm25_vs_emb_comparison():
    ...
    def get_baseline_precision(repr_type):
        ...
        for r in data["baseline"]:
            if r["type"] == "point_in_time" and r["representation"] == repr_type:
                target_year = "2015" if "2015" in r["desc"] else "2024" if "2024" in r["desc"] else "2023"
    ...
    ax1.bar(['BM25', 'Embeddings'], [precision_bm25, precision_emb], color=['#A9A9A9', '#4682B4'])
    ax1.set_title("Précision Baseline (Point-in-Time)\n% de documents de la bonne année")
    ax1.set_ylabel("Précision (%)")
    ax1.set_ylim(0, 100)

    # Graph 2 : Recency Score
    ax2.bar(['BM25 Baseline', 'Embeddings Mode B'], [avg_year_bm25, avg_year_emb], color=['#A9A9A9', '#2E8B57'])
