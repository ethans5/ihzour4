...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python evaluation/summarize_results.py evaluation/results/run_YYYYMMDD_HHMMSS.jsonl"
        )
        return
            ...
            writer.writerow(
                {
                    ...
                    "retrieved_chunk_ids": " | ".join(
                        r.get("retrieved_chunk_ids", [])[:5]
                    ),
                    "answer_preview": (
                        (r.get("answer", "")[:180] + "...") if r.get("answer") else ""
                    ),
                }
            )
