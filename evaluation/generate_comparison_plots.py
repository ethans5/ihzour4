...
def plot_baseline_comparison(data, corpus_name):
    ...
    bars = plt.bar(
        readable, avg_scores, color=["#A9A9A9", "#4682B4", "#D3D3D3", "#5F9EA0"]
    )
    plt.title(
        f"Baseline Temporal Precision (Point-in-Time Queries) - {corpus_name}\n(How often do they accidentally find the right year?)"
    )
...
def plot_hard_filter_success(data, corpus_name):
    ...
    fixed_emb_success = [
        r["success"]
        for r in hf_results
        if r["chunking"] == "fixed" and r["representation"] == "embeddings"
    ]
    ...
    child_emb_success = [
        r["success"]
        for r in hf_results
        if r["chunking"] == "child" and r["representation"] == "embeddings"
    ]
...
def plot_recency_effect(data, corpus_name):
    ...
    for r in recency_results:
        if r["representation"] != "embeddings":
            continue
    ...
    bars = plt.bar(
        ["Fixed + Emb", "Child + Emb"],
        [mean_year_fixed, mean_year_child],
        color=["lightgreen", "orange"],
    )
    plt.title(
        f"Avg. Retrieved Year for 'Current' Queries - {corpus_name}\n(α={target_alpha}, λ={target_lambda})"
    )
...
def generate_comparative_graphs():
    ...
    for corpus in ["UK", "US"]:
        ...
        try:
            ...
            try:
                with open(log_file, "a", encoding="utf-8") as log:
                    log.write(f"  plot_baseline_comparison done for {corpus}\n")
            except:
                pass
            ...
            try:
                with open(log_file, "a", encoding="utf-8") as log:
                    log.write(f"  plot_hard_filter_success done for {corpus}\n")
            except:
                pass
            ...
            try:
                with open(log_file, "a", encoding="utf-8") as log:
                    log.write(f"  plot_recency_effect done for {corpus}\n")
            except:
                pass
